...
class UserAccountStatus(DjangoChoices):
    ...
    EXCHANGE_ENABLED = ChoiceItem(
        1, "Stage1: Exchange Account Enabled"
    )  # 交易所账号激活
...
FiatServiceChargeMin = {FiatType.USD: (10000, 30, 0.003)}
...
class WithdrawStatus(DjangoChoices):
    ...
    VERIFY_FAILED = ChoiceItem(
        -4, "User Verify Failed"
    )  # 用户审核: 失败, 超过一天时间未确认状态
    #
    ...
    REVIEW_COMPLETED = ChoiceItem(
        4, "Admin Verify Completed"
    )  # 客服人工审核: Completed
    #
...
class FiatWithdrawFianceAllowStatus(DjangoChoices):
    ...
    OUT_MONEY_FAILED = ChoiceItem(
        -9, "Admin  Verify Pending"
    )  # 财务出款失败: Completed
...
class TransactionType(DjangoChoices):
    ...
    SWAP_ERC_TOKEN_FROM_TX_ADDR = ChoiceItem(
        2, "Swap ERC Token: From User Tx Address"
    )  # 矿工手续费(系统支付)
    SWAP_ERC_TOKEN_FROM_COLD_WALLET = ChoiceItem(
        3, "Swap ERC Token: from Cold Wallet"
    )  # 矿工手续费(系统支付)
...
class DepositType(DjangoChoices):
    ...
    SYS_REFUND = ChoiceItem(
        3, "Cancel Withdraw, SYS Refund"
    )  # 用户撤销提现, 系统退款(重新产生一笔充值)
