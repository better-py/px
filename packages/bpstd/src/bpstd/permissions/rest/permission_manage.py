...
from maneki.apps.user_role.utils.cache import get_permission_cache, make_key
...
method_map = {"GET": ApiPermissionType.List, "POST": ApiPermissionType.Create}
...
class StrictPermissionCheck(BasePermission):
    ...
    def check_role_permission_settings_from_db(path, method_code, role_id):
        permission = Permission.objects.filter(
            route_path=path, permission_code=method_code
        ).first()
        ...
        permission_setting = RolePermission.objects.filter(
